...
def LFR(params, data_sensitive, data_nonsensitive, y_sensitive,
    ...
    if LFR.n_steps % 500 == 0:
        print(f'n_steps: {LFR.n_steps} loss: {criterion}, L_x: {L_x}, L_y: {L_y}, L_z: {L_z}')

    if results:
        return yhat_sensitive, yhat_nonsensitive, M_nk_sensitive, M_nk_nonsensitive, x_n_hat_sensitive, x_n_hat_nonsensitive
    else:
        return criterion
