...
for ((i=0; i<N; i++)); do
    ...
    PYTHON_COMMAND="CUDA_VISIBLE_DEVICES=${CUDA_DEVICE} python -m sklearnex main.py -dataset ${DATASET} -configs configs/configs.cfg -all -runs 1 -seed $SEED >> log_seed_${SEED}.out 2>&1"
